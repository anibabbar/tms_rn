...
const VendorScreen = (props) => {
    ...
    useEffect(() => {
        getVendorList()
        EventEmitter.on("onAddVendor", onAddVendor1);

        return () => {
            EventEmitter.removeListener("onAddVendor", onAddVendor1);
        }
    }, [])
    ...
    const onDeleteVendor = async (rowMap, rowKey) => {
        ...
        let data = await ApiServices.PostApiCall(ApiEndpoint.DELETE_VENDOR + '/' + rowMap[rowKey].props.item.id, null, headers);
        if (!!data && !!data.status) {
            const newData = [...vendorList];
            const prevIndex = vendorList.findIndex(item => item.key === rowKey);
            newData.splice(prevIndex, 1);
            setVendorList(newData);
            Constants.showAlert.alertWithType('success', 'Success', 'Vendor deleted Successfully!');
        } else {
            Constants.showAlert.alertWithType('error', 'Error', data.message);
        }
        Constants.showLoader.hideLoader();
