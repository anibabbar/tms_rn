...
const ClientScreen = (props) => {
    ...
    useEffect(() => {
        getVendorList()
        EventEmitter.on("onAddClient", onAddClient);

        return () => {
            EventEmitter.removeListener("onAddClient", onAddClient);
        }
    }, [])
    ...
    const renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity
                ...
                {<Text style={styles.textCard}>Alt. Phone: {!!item.alt_phone ? item.alt_phone : '-'}</Text>}
    ...
    const onDeleteVendor = async (rowMap, rowKey) => {
        ...
        let data = await ApiServices.PostApiCall(ApiEndpoint.DELETE_CLIENT + '/' + rowMap[rowKey].props.item.id, null, headers);
        if (!!data && !!data.status) {
            const newData = [...vendorList];
            const prevIndex = vendorList.findIndex(item => item.key === rowKey);
            newData.splice(prevIndex, 1);
            setVendorList(newData);
            Constants.showAlert.alertWithType('success', 'Success', 'Client deleted Successfully!');
        } else {
            Constants.showAlert.alertWithType('error', 'Error', data.message);
        }
        Constants.showLoader.hideLoader();
