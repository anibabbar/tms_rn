...
const DocCatScreen = (props) => {
    ...
    useEffect(() => {
        getVendorList()
        EventEmitter.on("onAddDocCat", onAddDocCat);

        return () => {
            EventEmitter.removeListener("onAddDocCat", onAddDocCat);
        }
    }, [])
    ...
    const onAddVendor = () => {
        props.navigation.navigate('AddDocCat');
    }
    ...
    const renderItem = ({ item, index }) => {
        return (
            <TouchableOpacity
                ...
                <Text style={styles.textCard}>Type: {item.type}</Text>
            </TouchableOpacity>
        )
    }
    ...
    const onDeleteVendor = async (rowMap, rowKey) => {
        ...
        let data = await ApiServices.PostApiCall(ApiEndpoint.DELETE_CAT_DOC + '/' + rowMap[rowKey].props.item.id, null, headers);
        if (!!data && !!data.status) {
            const newData = [...vendorList];
            const prevIndex = vendorList.findIndex(item => item.key === rowKey);
            newData.splice(prevIndex, 1);
            setVendorList(newData);
            Constants.showAlert.alertWithType('success', 'Success', 'Doc category deleted Successfully!');
        } else {
            Constants.showAlert.alertWithType('error', 'Error', data.message);
        }
        Constants.showLoader.hideLoader();
    ...
    const deleteRow = (rowMap, rowKey) => {
        Alert.alert(
            "Delete Doc category",
            "Are you sure you want to delete doc category?",
            [
                {
                    text: "No",
                ...
                <View style={styles.headerView}>
                    <View style={styles.headerRow}>
                        ...
                        <View style={styles.titleContainer}>
                            <Text style={styles.title}>Doc Categories</Text>
                        </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(DocCatScreen);